        ...
        if not status_atual:
            ...
            try:
                ...
                while botao_boleto != "Emitir Boleto":
                    botao_boleto = WebDriverWait(navegador, 10).until(EC.visibility_of_element_locate((By.CSS_SELECTOR, f"#LicenciamentoExercicio > table > tbody > tr:nth-child({id_linha2}) > td > input[type=button]")))
                    id_linha2 += 1
                    if botao_boleto:
                        break
                    else:
