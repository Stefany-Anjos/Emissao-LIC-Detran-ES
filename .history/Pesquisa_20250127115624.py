...
def caminho_paraBoleto(pasta_baixou, pasta_saida, placa_atual):
    ...
    if arquivo in arquivos:
        caminho_arquivo = os.path.join(pasta_baixou, arquivo) # Define o caminho para salvar o arquivo
        novo_nome = os.path.join(pasta_saida, f"{placa_atual}.pdf") # Renomeia o arquivo com a placa
...
prefs = {
    "credentials_enable_service": False,  # Desativar salvar senhas
        ...
        if not status_atual:
            ...
            try:
                ...
                while botao_boleto != "Emitir Boleto":
                    botao_boleto = WebDriverWait(navegador, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, f"#LicenciamentoExercicio > table > tbody > tr:nth-child({id_linha2}) > td > input[type=button]")))
                    if botao_boleto:
                        break
                    else:
                        id_linha2 +=1
                ...
                botao_emitir = navegador.find_element(By.CSS_SELECTOR, f"#LicenciamentoExercicio > table > tbody > tr:nth-child({id_linha2}) > td > input[type=button]")
                botao_emitir.click()
                ...
                try:
                    ...
                    EC.element_to_be_clickable((By.ID, "GovBr"))
                    ).click()
                    ...
                    try:
                      # Fechar aviso de salvamento senha  
                      fechar_alerta = navegador.find_element(By.XPATH, '//button[contains(text(), "Nunca")]')  # Texto pode variar, como "Não" ou "Nunca"
                      fechar_alerta.click()
                    except TimeoutException:
                          print("Nenhum aviso de salvar senha encontrado.")
