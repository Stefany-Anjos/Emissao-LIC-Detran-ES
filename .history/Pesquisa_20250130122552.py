...
def login_gov():
    ...
    campoCPF = WebDriverWait(navegador, 60).until(
        EC.visibility_of_element_located(
            (By.CSS_SELECTOR, "#accountId")))

    # Insere o CPF no campo
    campoCPF.send_keys(os.getenv("LOGIN"))

    # Botão continuar
    botaoContinuarLogin = navegador.find_element(
        By.CSS_SELECTOR, "#enter-account-id")
    ...
    botaoEntrar = navegador.find_element(
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            tela_erroRenavam = WebDriverWait(navegador, 10).until(
                                EC.visibility_of_element_located((By.CSS_SELECTOR, "body > center > font")))
                ...
                while True:
                    try:
                        ...
                        if botao_boleto.get_attribute("value") == "Emitir Boleto":
                            ...
                            break
                        erro_404 = WebDriverWait(navegador, 10).until(EC.visibility_of_element_located(
                            (By.CSS_SELECTOR, "#content > div > fieldset > h2")))
                        erro_404 = erro_404.text
                        # Verifica se o texto obitido é 404 - file or directory not found.
                        if erro_404 == "404 - File or directory not found.":
                            guia_dados.cell(
                                row=index + 2, column=1, value="Erro ao emitir")
                            # Salva a planilha antes de continuar
                            planilha.save(caminho_planilha)
                        ...
                        guia_dados.cell(row=index + 2, column=1,
                                        value=f"{tela_erroRenavam}")
                        break
                try:
                    # Esperar tela de atenção para clicar no botão entrar com o GOV
                    botaoAcessarGOV = WebDriverWait(navegador, 15).until(
